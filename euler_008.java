...
class euler_008
{
  ...
  static long product(List<Integer> data)
  {
    long result = 1;
    for (int number : data)
      result *= number;
  ...
  static List<Integer> max_product(List<Integer> digits, int length)
  {
    long max = 0;
    ...
    while (start+length <= digits.size())
    {
      ...
      List<Integer> slice = digits.subList(start, start+length);
  ...
  // integer digits
  static List<Integer> toIntegers(String string)
  {
  ...
  public static void main(String[] args)
  {
    ...
    List<Integer> digits = toIntegers(number);
    // int[] lengths = {5, 7, 11, 13};
    int[] lengths = {4, 7, 10, 13};
    for (int length : lengths)
    {
      List<Integer> slice = max_product(digits, length);
      System.out.println("max of " + length + ": " + product(slice) + " from " + slice);
    }
  }
}
