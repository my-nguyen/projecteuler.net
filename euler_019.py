  ...
  # otherwise, must calculate leap year
    ...
    if (year % 4 != 0):
      ...
      if (year % 100 != 0):
        ...
        return 29 if (year % 400 == 0) else 28
...
for year in range(1900, 2001):
  ...
  for month in range(1, 13):
    ...
    # year accordingly
    annee = year + (1 if (month == 12) else 0)
    # print("offset: %d, 1st of %s of %d: %d" << %(offset, months[month], annee, (date+offset)))
    ...
    if ((date+offset) % 7 == 0 and annee != 1900):
      count += 1
      print("%s of %d" %(months[month], annee))
  # advance one year
  date += offset

print("%d Sundays fell on the first of the month during the twentieth century" %count)
